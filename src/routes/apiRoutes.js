...
// ===== PUT - Actualizar marcador en tiempo real (admin/bar) =====
router.put('/admin/update-score/:roomId', authMiddleware, async (req, res) => {
  try {
    const { roomId } = req.params;
    ...
    if (user.role !== 'admin' && user.role !== 'bar') {
      return res.status(403).json({
        success: false,
        message: 'No autorizado - Solo administradores pueden actualizar el marcador'
      });
    }
    ...
    if (room.fixture_id) {
      ...
      console.log(`✅ Marcador actualizado: ${goals_home} x ${goals_away}`);

      // 🔥 Opcional: Emitir evento via WebSocket si tienes implementado
      // io.emit(`score-update-${roomId}`, { goals_home, goals_away });
    }
...
// ===== PUT - Actualizar marcador (fallback HTTP) =====
router.put('/update-match-score/:roomId', authMiddleware, async (req, res) => {
  try {
    const { roomId } = req.params;
    const { goals_home, goals_away } = req.body;
